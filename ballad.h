...
typedef enum {BOOLEAN_FLAG, STRING_FLAG, INTEGER_FLAG} FlagType;
...
BalladCli ballad_init(int charc, char **charv, char *help) 
{
    return (BalladCli){help,  (BalladCommand *)malloc(5 * sizeof(BalladCommand)), 5, 0, charc, charv};
}
...
void ballad_add_command(BalladCli *cli, BalladCommand command) 
{
    ...
    {
        cli->capacity *= 2;
        cli->buffer = (BalladCommand *)realloc(cli->buffer, cli->capacity * sizeof(BalladCommand));
    }
...
char *ballad_get_arg(int charc, char **charv, int index) 
{
    // dunno why this function exists tbh i was trying to do the golang thingy
